    ...
    <div className="flex h-screen bg-[#faf7f2] font-sans text-stone-800 overflow-hidden selection:bg-[#8c6d53] selection:text-white">
      ...
      <aside className="w-64 bg-white/90 backdrop-blur-md border-r border-[#e8dfd5] flex-col shrink-0 hidden lg:flex z-20 relative shadow-sm">
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <IconComponent
                  size={19}
                  className={active ? "text-amber-300" : "text-stone-400"}
                />
                {item.name}
              </Link>
            );
        ...
        <div className="p-4 border-t border-[#f0eae1] space-y-1.5 bg-[#fcfaf7]/50">
          ...
          >
            <ShieldCheck size={17} className="text-[#8c6d53]" /> Reset Password
          </Link>
          <button
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          ...
          ></div>
          <aside className="w-68 bg-white h-full shadow-2xl relative z-10 flex flex-col animate-in slide-in-from-left-4 duration-200">
            <div className="h-20 flex items-center justify-between px-6 border-b border-[#e8dfd5]">
            ...
            <div className="p-4 border-t border-[#e8dfd5]">
              <button
                onClick={handleLogout}
                className="w-full flex items-center gap-3 px-3.5 py-2.5 text-xs font-semibold text-rose-700 hover:bg-rose-50 rounded-xl transition"
              >
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        {/* 🌐 TOPBAR Header */}
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-[#e8dfd5] flex items-center justify-between px-6 sm:px-8 shrink-0 z-10">
          {/* Left: Mobile Toggle & Page Title */}
          <div className="flex items-center gap-4">
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="p-2 text-stone-600 hover:bg-stone-100 rounded-xl lg:hidden border border-stone-200"
            >
              <Menu size={20} />
            </button>
            <div>
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            <div className="flex items-center gap-4 pl-4 sm:pl-6 sm:border-l border-[#e8dfd5]">
              {/* Notification Bell */}
              <button className="relative p-2.5 text-stone-600 hover:text-stone-900 transition rounded-xl hover:bg-[#f8f5f0] border border-stone-200/80">
                <Bell size={18} />
                <span className="absolute top-2 right-2 w-2 h-2 bg-amber-600 rounded-full ring-2 ring-white"></span>
              </button>
              ...
              <div className="flex items-center gap-3">
                <div className="text-right hidden sm:block">
                  ...
                  <p className="text-[10px] text-[#8c6d53] font-bold tracking-wider uppercase flex items-center justify-end gap-1 mt-0.5 font-mono">
                    <MapPin size={10} className="text-[#8c6d53]" /> Property
                    Admin
                  </p>
                </div>
