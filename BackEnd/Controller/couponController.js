...
exports.getAllCoupons = async (req, res) => {
  try {
    // Query parameters nikal rahe hain (search aur sorting ke liye)
    const { search, sortBy, order } = req.query;

    let query = {};

    // 1. Searching Logic (Coupon code ke basis par search karega)
    if (search) {
      const searchRegex = new RegExp(search, "i"); // Case-insensitive search
      query = {
        $or: [{ couponCode: searchRegex }, { discountType: searchRegex }],
      };
    }

    // 2. Sorting Logic (Default newest first rahega)
