...
const getPendingHotels = async (req, res) => {
  try {
    ...
    if (search && search.trim() !== "") {
      ...
      filter.$or = [{ hotelName: searchRegex }, { hotelEmail: searchRegex }];
    }
...
const getAllPublicHotels = async (req, res) => {
  try {
    ...
    // Base query: Sirf Approved hotels nikalne ke liye
    ...
    if (state) {
      query.$or = [
        ...
        {
          "city.districtId.stateId.stateName": {
    ...
    if (city) {
      query.$or = [
        ...
        { "city.cityName": { $regex: new RegExp(`^${city}$`, "i") } },
      ];
    }

    // 3. Property Type Filter
    if (propertyType) {
      query.hotelType = { $regex: new RegExp(`^${propertyType}$`, "i") };
    }
    ...
    // Main Query execution with population, sorting, and pagination
    const hotels = await hotelModel
      .find(query)
      .populate({
        path: "city",
...
// =========================================================================
// =========================================================================
// GET ALL HOTELS (With Search, Filtering & Sorting)
// =========================================================================
const getAllHotels = async (req, res) => {
    try {
        const { search, status, sortBy, city, page = 1, limit = 10 } = req.query;
        let query = {};
        ...
        const totalHotelsCount = await hotelModel.countDocuments(query);
        const totalPages = Math.ceil(totalHotelsCount / limitNumber);
