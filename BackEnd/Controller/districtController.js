...
//Get Active (with Search, State Filter, Sort, & Pagination)
const getAllDistricts = async (req, res) => {
  try {
    const { search, stateId, sort, page = 1, limit = 10 } = req.query;

    let query = { status: "Active" };

    // 1. Search Logic
    if (search) {
      query.districtName = { $regex: search, $options: "i" };
    }

    // 2. State Filter Logic
    if (stateId) {
...
//Get Inactive (with Search, State Filter, Sort, & Pagination)
